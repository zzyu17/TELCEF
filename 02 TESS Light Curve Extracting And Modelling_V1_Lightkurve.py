...
if clip:
    ...
    j = 1 # count the sub-step
    lc_clipped_baseline = lc_flattened_baseline.remove_outliers(sigma=sigma)
    ...
    ax_lc_clipped_baseline.set_title(f"{name} Sector {sector} {pipeline} {sigma} Sigma Clipped Baseline Exptime={exptime}s")
    lc_clipped_baseline_plot.figure.tight_layout()
    lc_clipped_baseline_plot.figure.savefig(processed_lightcurve_plots_exptime_parent_dir + f"/{i:02}-{j:01} {name} Sector {sector} {pipeline} {sigma} Sigma Clipped Baseline Exptime={exptime}s.png")


    # Retrieve and plot the clipped light curve
    j += 1 # count the sub-step
    lc_clipped = lc_clipped_baseline.append(lc_flattened_transit)  # append the in-transit NaNs-removed raw light curve to the clipped baseline to retrieve the clipped light curve
    ...
    ax_lc_clipped.set_title(f"{name} Sector {sector} {pipeline} {sigma} Sigma Clipped Light Curve Exptime={exptime}s")
    lc_clipped_plot.figure.tight_layout()
    lc_clipped_plot.figure.savefig(processed_lightcurve_plots_exptime_parent_dir + f"/{i:02}-{j:01} {name} Sector {sector} {pipeline} {sigma} Sigma Clipped Light Curve Exptime={exptime}s.png")
...
methodology_result_file.write(      f"Raw Light Curve CDPP: {lc_raw_cdpp}\n"
                                    ...
                                    f"BLS Fitted NaNs-Removed Raw Light Curve Period (Days): {period_raw_nans_removed}\n"
                                    f"BLS Fitted NaNs-Removed Raw Light Curve Epoch Time (BTJD): {epoch_time_raw_nans_removed}\n"
                                    f"BLS Fitted NaNs-Removed Raw Light Curve Transit Duration (Days): {transit_duration_raw_nans_removed}\n"
                                    f"BLS Fitted NaNs-Removed Raw Light Curve Transit Duration (Cadences): {transit_duration_raw_nans_removed_in_cadence}\n"
                                    f"BLS Fitted NaNs-Removed Raw Light Curve Transit Depth: {transit_depth_raw_nans_removed}\n"
