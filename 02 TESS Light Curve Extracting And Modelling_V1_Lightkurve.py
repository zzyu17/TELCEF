...
##### Define the parameters corresponding to the exptime #####
exptime_array = [600, 120, 20]

flatten_list = [True] * 3
flatten_window_proportion_array = [0.02,0.05,0.05]

clip_list = [True] * 3
sigma_array = [3] * 3

bin_list = [True] * 3
cadence_bin_size_array = [2.5,10,50]


# Process the light curve of each exposure time
for j in range(len(exptime_array)):
    exptime = exptime_array[j]
    # Define the exptime-specified directory
    ...
    lc_raw_plot.figure.savefig(processed_lightcurve_plots_exptime_parent_dir + f"/{i:02} {name} Sector {sector} {pipeline} Raw Light Curve Exptime={exptime}s.png")

    lc_raw_remove_nans = lc_raw.remove_nans()
    ...
    flatten = flatten_list[j]
    if flatten:
        flatten_window_proportion = flatten_window_proportion_array[j]
    ...
    bin = bin_list[j]
    if bin:
        cadence_bin_size = cadence_bin_size_array[j]
